                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th>إجمالي الإيداعات</th>
                                    <th>إجمالي السحوبات</th>
                                    <th>إجمالي أرباح المهام</th>
                                    <th>إجمالي أرباح الإحالات</th>
                                </tr>
                            </thead>
...
def approve_transaction():
    ...
    if transaction.status != "pending":
        return jsonify({"error": "لا يمكن قبول معاملة ليست قيد الانتظار"}), 400
    ...
    if transaction.type == "deposit":
        ...
        # For withdrawals, balance should have been checked at request time
...
def reject_transaction():
    ...
    if transaction.status != "pending":
        return jsonify({"error": "لا يمكن رفض معاملة ليست قيد الانتظار"}), 400

    transaction.status = "rejected"
    # If it was a deposit, and we reject it, we might need to reverse the balance if it was already added.
    # For simplicity, assuming balance is only updated on approval.
    db.session.commit()
...
def get_account_info():
    ...
    for user in users:
        ...
        total_withdrawals = sum(t.amount for t in user.transactions if t.type == \"withdraw\" and t.status == \"completed\")
        total_task_earnings = sum(t.amount for t in user.transactions if t.type == \"task_reward\")
        total_referral_earnings = sum(t.amount for t in user.transactions if t.type == \"referral_commission\")
